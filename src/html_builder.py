...
TIER_CFG = {
    "ÉLITE": {"color": "#92720A", "bg": "#FBF5E0", "border": "#D4AA3A",
...
def _form_dots(form: str) -> str:
    ...
    for ch in form.upper().replace(",", "").replace(" ", "")[:6]:
        ...
        out.append(f'<span class="{cls} rounded-circle d-inline-block me-1" '
...
def _section_hd(title: str, icon: str) -> str:
    return (f'<div class="d-flex align-items-center gap-2 mt-4 mb-2 pb-2 border-bottom border-warning-subtle">'
            ...
            f'<span class="fw-bold text-uppercase small" style="letter-spacing:1.5px;color:#92720A">'
            f'{_e(title)}</span></div>')
...
def _modal(r: dict, idx: int) -> str:
    ...
    bp_p1   = r.get("betplay_impl_prob_1", 0)
    bp_px   = r.get("betplay_impl_prob_x", 0)
    ...
    vtag = ('<span class="badge bg-warning text-dark ms-1">'
            '<i class="bi bi-gem me-1"></i>VALUE BET</span>'
            if r.get("_value") else "")

    # Elo
    elo_html = ""
    if elo_l or elo_v:
        elo_ok2 = (elo_d and elo_d > 0 and is_local) or (elo_d and elo_d < 0 and not is_local)
        elo_html = f"""
    ...
    if bp_ok and bp_o1:
        ...
        agree_icon = ("bi-check-circle-fill text-success" if agreement <= 10
                      else "bi-exclamation-triangle-fill text-warning" if agreement <= 20
                      else "bi-x-circle-fill text-danger")
        agree_txt  = ("Alineado" if agreement <= 10
                      else "Discrepancia leve" if agreement <= 20
                      else "Contradice")
        bp_html = f"""
        {_section_hd("BetPlay — Cuotas en vivo", "bi-currency-dollar")}
        <div class="row g-2 mb-2">
          <div class="col-4">
            <div class="card border-0 bg-primary-subtle text-center py-2 px-1">
    ...
    circ  = 125.66
    dash  = circ * conf / 100
    rcolors = {"ÉLITE": "#92720A", "ALTA": "#1A5C96", "MEDIA": "#6A7A8A", "BAJA": "#B0BAC8"}
    rc    = rcolors.get(tier, "#6A7A8A")
    ...
    <div class="row g-3 mb-1">
      <div class="col-4">
        <div class="card border-0 text-center py-3"
          style="background:{tc['bg']};border:1px solid {tc['border']}!important">
          <div class="small fw-bold mb-1" style="color:{acc};letter-spacing:1px">CONFIANZA</div>
          <div style="font-size:2rem;font-weight:900;color:{acc};line-height:1">
            {conf}<span style="font-size:1rem">%</span>
          </div>
        </div>
      </div>
      <div class="col-4">
      ...
      <div class="col-4">
        <div class="card border-0 bg-light text-center py-3">
          ...
          <div style="font-size:2rem;font-weight:900;line-height:1">
            +{margin}<span style="font-size:1rem">pts</span>
          </div>
        </div>
      </div>
    </div>
...
def _card(idx: int, r: dict) -> str:
    ...
    elo_fav = r.get("elo_favorito")
    elo_ok  = elo_fav and (
        (elo_fav == "LOCAL" and is_local) or
        (elo_fav == "VISITANTE" and not is_local)
    )
    elo_badge = ""
    if elo_fav:
        bs = "success" if elo_ok else "secondary"
        elo_badge = (f'<span class="badge bg-{bs}-subtle text-{bs}-emphasis border border-{bs}-subtle small">'
    ...
    vtag = ('<span class="badge bg-warning text-dark small">'
            '<i class="bi bi-gem me-1"></i>VALUE</span>' if value else "")
    ...
    <div class="row g-0 align-items-stretch">
      ...
      <div class="col-auto d-flex flex-column align-items-center justify-content-center gap-2 px-3 py-3"
        ...
        <span class="badge px-2 py-1 fw-bold"
          style="background:{acc}18;color:{acc};border:1px solid {acc}40;font-size:11px">{pick}</span>
      </div>
      ...
      <div class="col-auto d-none d-md-flex flex-column justify-content-center px-3 py-3"
        ...
        <div class="d-flex align-items-center gap-2 mb-2">
          ...
          <span class="small fw-{'bold' if is_local else 'normal'}"
            style="width:36px;text-align:right;color:{acc if is_local else '#8A9BAC'}">{p1}%</span>
        </div>
        <div class="d-flex align-items-center gap-2 mb-2">
        ...
        <div class="d-flex align-items-center gap-2">
          ...
          <span class="small fw-{'bold' if not is_local else 'normal'}"
            style="width:36px;text-align:right;color:{acc if not is_local else '#8A9BAC'}">{p2}%</span>
        </div>
...
def _stat_card(val: Any, label: str, icon: str, color: str = "#92720A") -> str:
    return (f'<div class="col">'
            f'<div class="card border-0 shadow-sm text-center py-3 px-2 h-100">'
...
<style>
:root{{--bs-body-bg:#F0F3F8;--bs-body-font-family:'Inter',sans-serif;--gold:#92720A;--gold-lt:#FBF5E0}}
...
@media(max-width:576px){{.hero-section .display-5{{font-size:1.75rem}}}}
</style>
</head>
<body>
    ...
    <div class="modal-content border-0 shadow-lg">
      <div class="modal-body text-center p-5">
        ...
        <button type="button" class="btn btn-warning fw-bold px-4"
          data-bs-dismiss="modal" id="enterBtn">
...
<nav class="navbar navbar-expand-lg sticky-top shadow-sm">
  <div class="container-xl">
    <a class="navbar-brand d-flex align-items-center gap-2" href="#">
      ...
      <div>
        <div style="font-size:14px;font-family:'Sora',sans-serif;font-weight:900;line-height:1.1">SportAnalysis</div>
...
<div class="container-xl py-4">
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
    ...
    <div class="d-flex gap-2 flex-wrap">
      ...
      <button class="btn btn-sm btn-outline-warning rounded-pill tier-btn" data-t="ÉLITE">
  ...
  <div class="card border-0 shadow-sm mt-4">
    <div class="card-body">
      ...
      <div class="d-flex flex-wrap gap-3 align-items-center">
        <div><span class="badge bg-warning text-dark me-1"><i class="bi bi-trophy-fill"></i> ÉLITE</span><span class="small text-muted">Confianza ≥ 65</span></div>
        <div><span class="badge bg-primary me-1"><i class="bi bi-graph-up-arrow"></i> ALTA</span><span class="small text-muted">50–64</span></div>
        <div><span class="badge bg-secondary me-1"><i class="bi bi-dash-circle"></i> MEDIA</span><span class="small text-muted">35–49</span></div>
        <div><span class="badge bg-warning text-dark me-1"><i class="bi bi-gem"></i> VALUE</span><span class="small text-muted">Prob ≥ 50% y ventaja ≥ 15pts</span></div>
        <div class="w-100"><small class="text-muted"><i class="bi bi-info-circle me-1"></i>Score = prob. victoria (60%) + margen (40%) + claridad del pick (20%) ± Club Elo ± BetPlay</small></div>
      </div>
    </div>
  </div>
...
(function(){{
  ...
  function render(){{
    ...
    allCards.forEach(c=>{{
      ...
      if(ok){{c.style.animationDelay=(v*0.04)+'s';v++;}}
    }});
    document.getElementById('empty-state').classList.toggle('d-none',v>0);
  }}
